...
rtm.on('message', (message) => {
  ...
  console.log(message.subtype, message.bot_id);

  if (message.subtype && message.subtype === 'bot_message' || message.bot_id) {
    web.bots.info({
      token: clientToken,
      bot: message.user,
    })
    .then((res) => {
      web.chat.postMessage({
        ...
        attachments: [{
          ...
          text: message.attachments.shift().text,
        }]
      }).catch(console.error);
    }).catch(console.error);
  } else {
...
if (!module.parent) {
  ...
  console.log(`${npm_package_name} @${npm_package_version} is running with:
    clientId: ${clientId}
